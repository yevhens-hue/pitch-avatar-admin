...
function SourceProjectsContent() {
  ...
  const displayProjects = typeParam 
    ? projects.filter(p => p.type === typeParam)
    : projects

  const activeWizards = typeParam
    ? WIZARDS.filter(w => w.type === typeParam)
    : WIZARDS
    ...
    <div className="px-8 py-8 max-w-6xl">
      ...
      <div>
        <h2 className="text-sm font-semibold text-slate-900 uppercase tracking-wide mb-4">
          Available Source Projects ({displayProjects.length})
        </h2>
